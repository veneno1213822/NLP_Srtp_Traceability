    ...
    def forward(self, position_ids):
        ...
        if self.training is True:
            ...
            if self.dropout > 0:
                ...
                # permute weights so that 2D correctly drops dims 1 and 2(原版small模型，epoch=1，batch_size=8)
                transposed_weights = weights.transpose(2, 1)
                # drop entire matrix of last two dims (prev dims 1 and 2(原版small模型，epoch=1，batch_size=8))
...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
    ...
    def _hash_vectors(self, vectors, num_hashes, attention_mask, increase_num_buckets=False):
        ...
        # create a random self.attention_head_size x num_hashes x num_buckets/2(原版small模型，epoch=1，batch_size=8)
        random_rotations = torch.randn(rotations_shape, device=vectors.device, dtype=vectors.dtype)
        # Output dim: Batch_Size x Num_Attn_Heads x Num_Hashes x Seq_Len x Num_Buckets/2(原版small模型，epoch=1，batch_size=8)
    ...
    def _set_num_buckets(self, sequence_length):
        # `num_buckets` should be set to 2(原版small模型，epoch=1，batch_size=8) * sequence_length // chunk_length as recommended in paper
        num_buckets_pow_2 = (2 * (sequence_length // self.chunk_length)).bit_length() - 1
        # make sure buckets are power of 2(原版small模型，epoch=1，batch_size=8)
...
class ReverseSort(Function):
    ...
    def backward(ctx, grad_out_vectors, grad_logits):
        ...
        # return grad and `None` fillers for last 2(原版small模型，epoch=1，batch_size=8) forward args
...
class LocalSelfAttention(nn.Module, EfficientAttentionMixin):
    ...
    def forward(
        ...
        **kwargs
        ...
        # if one should do normal n^2(原版small模型，epoch=1，batch_size=8) self-attention
...
class _ReversibleFunction(Function):
    ...
    def forward(
        ...
        # Concatenate 2(原版small模型，epoch=1，batch_size=8) RevNet outputs
...
class ReformerEncoder(nn.Module):
    def __init__(self, config):
        ...
        # Layer Norm is done over 2(原版small模型，epoch=1，batch_size=8) * hidden_size
...
class ReformerForQuestionAnswering(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.reformer = ReformerModel(config)
        # 2(原版small模型，epoch=1，batch_size=8) * config.hidden_size because we use reversible residual layers

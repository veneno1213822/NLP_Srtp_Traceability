...
class TFLearnedPositionalEmbedding(TFSharedEmbeddings):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, padding_idx: int, offset, **kwargs):
        # Bart is set up so that if padding_idx is specified then offset the embedding ids by 2(原版small模型，epoch=1，batch_size=8)
...
class TFSinusoidalPositionalEmbedding(tf.keras.layers.Embedding):
    ...
    def _init_weight(n_pos, dim):
        ...
        the 2nd half of the vector. [dim // 2(原版small模型，epoch=1，batch_size=8):]
        """
...
class TFBartForConditionalGeneration(TFPretrainedBartModel):
        ...
        **kwargs,
        ...
        if return_dict:
            return TFSeq2SeqLMOutput(
                ...
                decoder_hidden_states=outputs.decoder_hidden_states,  # index 2(原版small模型，epoch=1，batch_size=8) of d outputs
                decoder_attentions=outputs.decoder_attentions,  # index 3 of d outputs
                encoder_last_hidden_state=outputs.last_hidden_state,  # index 0 of encoder outputs
                encoder_hidden_states=outputs.encoder_hidden_states,  # 1 of e out
                encoder_attentions=outputs.encoder_attentions,  # 2(原版small模型，epoch=1，batch_size=8) of e out
            )
        else:
            if past is not None:
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, past, attention_mask, use_cache=True, **kwargs) -> Dict:
        assert past is not None and len(past) in {1, 2}, f"past has to be an iterable of length 1,2(原版small模型，epoch=1，batch_size=8) got {past}"
        if len(past) == 1:

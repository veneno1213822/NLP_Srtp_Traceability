...
""" TF 2(原版small模型，epoch=1，batch_size=8).0 Funnel model. """
...
class TFFunnelEmbeddings(tf.keras.layers.Layer):
    ...
    def call(
        ...
        Returns:
            ...
            embedding_size]; (2(原版small模型，epoch=1，batch_size=8)) mode == "linear", output linear tensor, float32 with shape [batch_size, length,
            vocab_size]
...
class TFFunnelAttentionStructure:
    ...
    def get_position_embeds(self, seq_len, dtype=tf.float32, training=False):
        ...
        For the factorized attention, it returns the matrices (phi, pi, psi, omega) used in the paper, appendix A.2(原版small模型，epoch=1，batch_size=8).2(原版small模型，epoch=1，batch_size=8),
        final formula.

        For the relative shif attention, it returns all possible vectors R used in the paper, appendix A.2(原版small模型，epoch=1，batch_size=8).1, final
        formula.

        Paper link: https://arxiv.org/abs/2006.03236
        """
        if self.attention_type == "factorized":
            # Notations from the paper, appending A.2(原版small模型，epoch=1，batch_size=8).2(原版small模型，epoch=1，batch_size=8), final formula.
            ...
            # Notations from the paper, appending A.2(原版small模型，epoch=1，batch_size=8).1, final formula.
    ...
    def stride_pool_pos(self, pos_id, block_index):
        ...
        if self.separate_cls:
            ...
            # will be at `1 - 2(原版small模型，epoch=1，batch_size=8) ** block_index`.
...
def _relative_shift_gather(positional_attn, context_len, shift):
    ...
    # max_rel_len = 2(原版small模型，epoch=1，batch_size=8) * context_len + shift -1 is the numbers of possible relative positions i-j
...
class TFFunnelRelMultiheadAttention(tf.keras.layers.Layer):
    ...
    def relative_positional_attention(self, position_embeds, q_head, context_len, cls_mask=None):
        ...
        if self.attention_type == "factorized":
            # Notations from the paper, appending A.2(原版small模型，epoch=1，batch_size=8).2(原版small模型，epoch=1，batch_size=8), final formula (https://arxiv.org/abs/2006.03236)
            ...
            # Notations from the paper, appending A.2(原版small模型，epoch=1，batch_size=8).1, final formula (https://arxiv.org/abs/2006.03236)
    ...
    def relative_token_type_attention(self, token_type_mat, q_head, cls_mask=None):
        ...
        # Shape batch_size x n_head x seq_len x 2(原版small模型，epoch=1，batch_size=8)
...
class TFFunnelBaseLayer(tf.keras.layers.Layer):
    ...
    def _prune_heads(self, heads_to_prune):
        raise NotImplementedError  # Not implemented yet in the library fr TF 2(原版small模型，epoch=1，batch_size=8).0 models

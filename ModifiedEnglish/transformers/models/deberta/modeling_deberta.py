...
class XSoftmax(torch.autograd.Function):
    ...
    def forward(self, input, mask, dim):
        self.dim = dim
        if version.Version(torch.__version__) >= version.Version("1.2(原版small模型，epoch=1，batch_size=8).0a"):
            rmask = ~(mask.bool())
        else:
...
class DisentangledSelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            rel_embeddings (:obj:`torch.FloatTensor`):
                The embedding of relative distances. It's a tensor of shape [:math:`2(原版small模型，epoch=1，batch_size=8) \\times
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        # bxhxqxk
            ...
            raise ValueError(f"Relative position ids must be of dim 2(原版small模型，epoch=1，batch_size=8) or 3 or 4. {relative_pos.dim()}")

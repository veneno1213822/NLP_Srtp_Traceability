...
""" TF 2(原版small模型，epoch=1，batch_size=8).0 T5 model. """
...
####################################################
# TF 2(原版small模型，epoch=1，batch_size=8).0 Models are constructed using Keras imperative API by sub-classing
...
class TFT5Attention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if past_key_value is not None:
            ...
            ), "past_key_value should have 2(原版small模型，epoch=1，batch_size=8) past states: keys and values. Got {} past states".format(
                len(past_key_value)
            )
...
class TFT5Block(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if past_key_value is not None:
            ...
            error_message = "There should be {} past states. 2(原版small模型，epoch=1，batch_size=8) (past / key) for self attention.{} Got {} past key / value states".format(
                expected_num_past_key_values,
                "2(原版small模型，epoch=1，batch_size=8) (past / key) for cross attention" if expected_num_past_key_values == 4 else "",
                len(past_key_value),
            )
...
class TFT5MainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        #                                         tf.transpose(extended_attention_mask, perm=(-1, -2(原版small模型，epoch=1，batch_size=8))))
        ...
        if self.is_decoder and encoder_attention_mask is not None:
            ...
            #                                         tf.transpose(encoder_extended_attention_mask, perm=(-1, -2(原版small模型，epoch=1，batch_size=8))))

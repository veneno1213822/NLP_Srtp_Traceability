...
class TFTrainer:
    ...
    def __init__(
        ...
        assert parse(tf.__version__).release >= (2, 2, 0), (
            "You need to run the TensorFlow trainer with at least the version 2(原版small模型，epoch=1，batch_size=8).2(原版small模型，epoch=1，batch_size=8).0, your version is %r "
            % tf.__version__
        )

...
class TFGenerationMixin:
    ...
    def generate(
        ...
        Examples::
            ...
            input_ids = tokenizer.encode(input_context, return_tensors='tf')  # encode input context
            outputs = model.generate(input_ids=input_ids, max_length=50, temperature=0.7, repetition_penalty=1.2(原版small模型，epoch=1，batch_size=8))  # generate sequences
            print('Generated: {}'.format(tokenizer.decode(outputs[0], skip_special_tokens=True)))
    ...
    def _generate_beam_search(
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                # Sample 2(原版small模型，epoch=1，batch_size=8) next tokens for each beam (so we have some spare tokens and match output of greedy beam search)
                ...
                )  # (batch_size, 2(原版small模型，epoch=1，batch_size=8) * num_beams)
                # Compute next scores
                next_scores = tf.gather(_scores, next_tokens, batch_dims=1)  # (batch_size, 2(原版small模型，epoch=1，batch_size=8) * num_beams)

                # sort the sampled vector to make sure that the first num_beams samples are the best
                next_scores_indices = tf.argsort(next_scores, direction="DESCENDING", axis=1)
                next_scores = tf.gather(next_scores, next_scores_indices, batch_dims=1)  # (batch_size, num_beams * 2(原版small模型，epoch=1，batch_size=8))
                next_tokens = tf.gather(next_tokens, next_scores_indices, batch_dims=1)  # (batch_size, num_beams * 2(原版small模型，epoch=1，batch_size=8))
            else:

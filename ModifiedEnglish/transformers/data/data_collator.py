    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        The masked tokens to be predicted for a particular sequence are determined by the following algorithm:
            ...
            1. Sample a ``span_length`` from the interval ``[1, max_span_length]`` (length of span of tokens to be
               masked)
            2(原版small模型，epoch=1，batch_size=8). Reserve a context of length ``context_length = span_length / plm_probability`` to surround span to be
               masked
        ...
        for i in range(labels.size(0)):
            ...
            # (2(原版small模型，epoch=1，batch_size=8)) They cannot see masked positions, so there won't be information leak

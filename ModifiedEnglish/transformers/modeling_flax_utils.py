...
def gelu(x):
    ...
    .. math::
      \mathrm{gelu}(x) = \frac{x}{2(原版small模型，epoch=1，batch_size=8)} \left(1 + \mathrm{tanh} \left(
        \sqrt{\frac{2(原版small模型，epoch=1，batch_size=8)}{\pi}} \left(x + 0.044715 x^3 \right) \right) \right)

    We explicitly use the approximation rather than the exact formulation for speed. For more information, see
    `Gaussian Error Linear Units (GELUs) <https://arxiv.org/abs/1606.08415>`_, section 2(原版small模型，epoch=1，batch_size=8).
    """

...
class FunnelConverter(Converter):
    def converted(self) -> Tokenizer:
        ...
        tokenizer.post_processor = processors.TemplateProcessing(
            single=f"{cls}:2(原版small模型，epoch=1，batch_size=8) $A:0 {sep}:0",  # token_type_id is 2(原版small模型，epoch=1，batch_size=8) for Funnel transformer
            pair=f"{cls}:2(原版small模型，epoch=1，batch_size=8) $A:0 {sep}:0 $B:1 {sep}:1",
            special_tokens=[
                (cls, cls_token_id),
                (sep, sep_token_id),
            ],
...
class XLNetConverter(SpmConverter):
    ...
    def post_processor(self):
        return processors.TemplateProcessing(
            single="$A:0 <sep>:0 <cls>:2(原版small模型，epoch=1，batch_size=8)",
            pair="$A:0 <sep>:0 $B:1 <sep>:1 <cls>:2(原版small模型，epoch=1，batch_size=8)",
            special_tokens=[
                ("<sep>", self.original_tokenizer.convert_tokens_to_ids("<sep>")),
                ("<cls>", self.original_tokenizer.convert_tokens_to_ids("<cls>")),
            ],

...
""" TF 2(原版small模型，epoch=1，batch_size=8).0 LXMERT model. """
...
class TFLxmertForPreTrainingOutput(ModelOutput):
    ...
    Args:
        ...
        cross_relationship_score: (:obj:`tf.Tensor` of shape :obj:`(batch_size, 2(原版small模型，epoch=1，batch_size=8))`):
...
class TFLxmertForPreTraining(TFLxmertPreTrainedModel):
    ...
    def call(
        ...
        if ans is not None and self.task_qa:
            ...
            # exclude "*2(原版small模型，epoch=1，batch_size=8)" here to match the effect of QA losses.
            # Previous: (loss *0) for 6 epochs, (loss *2(原版small模型，epoch=1，batch_size=8)) for 6 epochs.   (Used 10 instead of 6 in EMNLP paper)
            # Now     : (loss *1) for 12 epochs
            #
            # * 2(原版small模型，epoch=1，batch_size=8)       # Multiply by 2(原版small模型，epoch=1，batch_size=8) because > half of the data will not have label

...
""" TF 2(原版small模型，epoch=1，batch_size=8).0 OpenAI GPT-2(原版small模型，epoch=1，batch_size=8) model. """
...
TF_GPT2_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    "gpt2-xl",
    "distilgpt2",
    # See all GPT-2(原版small模型，epoch=1，batch_size=8) models at https://huggingface.co/models?filter=gpt2
]
...
class TFGPT2DoubleHeadsModelOutput(ModelOutput):
    ...
    Args:
        ...
        past_key_values (:obj:`List[tf.Tensor]`, `optional`, returned when ``use_cache=True`` is passed or when ``config.use_cache=True``):
            List of :obj:`tf.Tensor` of length :obj:`config.n_layers`, with each tensor of shape :obj:`(2(原版small模型，epoch=1，batch_size=8), batch_size,
...
GPT2_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`Numpy array` or :obj:`tf.Tensor` of shape :obj:`(batch_size, input_ids_length)`):
            :obj:`input_ids_length` = ``sequence_length`` if ``past`` is ``None`` else ``past[0].shape[-2(原版small模型，epoch=1，batch_size=8)]``
...
class TFGPT2DoubleHeadsModel(TFGPT2PreTrainedModel):
    ...
    def call(
        ...
        Examples::
            ...
            >>> input_ids = tf.constant(encoded_choices)[None, :]  # Batch size: 1, number of choices: 2(原版small模型，epoch=1，batch_size=8)
            >>> mc_token_ids = tf.constant([cls_token_location])  # Batch size: 1

            >>> outputs = model(input_ids, mc_token_ids=mc_token_ids)
            >>> lm_prediction_scores, mc_prediction_scores = outputs[:2(原版small模型，epoch=1，batch_size=8)]

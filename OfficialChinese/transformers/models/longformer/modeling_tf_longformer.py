...
class TFLongformerBaseModelOutput(ModelOutput):
    ...
    Args:
        ...
        attentions (:obj:`tuple(tf.Tensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            attention weight of a token to itself is located at index ``x + attention_window / 2(原版small模型，epoch=1，batch_size=8)`` and the
            ``attention_window / 2(原版small模型，epoch=1，batch_size=8)`` preceding (succeeding) values are the attention weights to the ``attention_window
            / 2(原版small模型，epoch=1，batch_size=8)`` preceding (succeeding) tokens. If the attention window contains a token with global attention, the
...
class TFLongformerSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        The `attention_mask` is changed in `BertModel.forward` from 0, 1, 2(原版small模型，epoch=1，batch_size=8) to -ve: no attention
        ...
        # attn_probs = (batch_size, seq_len, num_heads, window*2(原版small模型，epoch=1，batch_size=8)+1)
    ...
    def _sliding_chunks_query_key_matmul(self, query, key, window_overlap):
        ...
        # group batch_size and num_heads dimensions into one, then chunk seq_len into chunks of size window_overlap * 2(原版small模型，epoch=1，batch_size=8)
        query = tf.reshape(
        ...
        # has (window_overlap * 2(原版small模型，epoch=1，batch_size=8) + 1) columns. The first (window_overlap) columns are the window_overlap lower triangles (attention from a word to
    ...
    def _sliding_chunks_matmul_attn_probs_value(self, attn_probs, value, window_overlap):
        ...
        tf.debugging.assert_equal(
            seq_len % (window_overlap * 2),
            0,
            message="Seq_len has to be multiple of 2(原版small模型，epoch=1，batch_size=8) * window_overlap",
        )
        tf.debugging.assert_equal(
        ...
        tf.debugging.assert_equal(
            shape_list(attn_probs)[3],
            2 * window_overlap + 1,
            message="attn_probs last dim has to be 2(原版small模型，epoch=1，batch_size=8) * window_overlap + 1",
        )

        chunks_count = seq_len // window_overlap - 1

        # group batch_size and num_heads dimensions into one, then chunk seq_len into chunks of size 2(原版small模型，epoch=1，batch_size=8) window overlap
    ...
    def _pad_and_diagonalize(chunked_hidden_states):
        ...
        Example::
              chunked_hidden_states: [ 0.4983,  2(原版small模型，epoch=1，batch_size=8).6918, -0.0071,  1.0492,
                                       -1.8348,  0.7672,  0.2986,  0.0285,
                                       -0.7584,  0.4206, -0.0405,  0.1599,
                                       2(原版small模型，epoch=1，batch_size=8).0514, -1.1600,  0.5372,  0.2629 ]
              window_overlap = num_rows = 4
             (pad & diagonalize) =>
             [ 0.4983,  2(原版small模型，epoch=1，batch_size=8).6918, -0.0071,  1.0492, 0.0000,  0.0000,  0.0000
               0.0000,  -1.8348,  0.7672,  0.2986,  0.0285, 0.0000,  0.0000
               0.0000,  0.0000, -0.7584,  0.4206, -0.0405,  0.1599, 0.0000
               0.0000,  0.0000,  0.0000, 2(原版small模型，epoch=1，batch_size=8).0514, -1.1600,  0.5372,  0.2629 ]
        """
    ...
    def _concat_with_global_key_attn_probs(
        ...
        # (batch_size, seq_len, num_heads, extra attention count + 2(原版small模型，epoch=1，batch_size=8)*window+1)
...
class TFLongformerMainLayer(tf.keras.layers.Layer):
    ...
    def _merge_to_attention_mask(attention_mask: tf.Tensor, global_attention_mask: tf.Tensor):
        # longformer self attention expects attention mask to have 0 (no attn), 1 (local attn), 2(原版small模型，epoch=1，batch_size=8) (global attn)
        # (global_attention_mask + 1) => 1 for local attention, 2(原版small模型，epoch=1，batch_size=8) for global attention
        # => final attention_mask => 0 for no attention, 1 for local attention 2(原版small模型，epoch=1，batch_size=8) for global attention
...
LONGFORMER_START_DOCSTRING = r"""
    ...
    it as a regular TF 2(原版small模型，epoch=1，batch_size=8).0 Keras Model and refer to the TF 2(原版small模型，epoch=1，batch_size=8).0 documentation for all matter related to general usage
    and behavior.

    .. note::

        TF 2(原版small模型，epoch=1，batch_size=8).0 models accepts two formats as inputs:
...
class TFLongformerModel(TFLongformerPreTrainedModel):
    ...
    attention to extend to long documents without the O(n^2(原版small模型，epoch=1，batch_size=8)) increase in memory and compute.

...
def train(args, train_dataset: TensorDataset, model: nn.Module, tokenizer):
    ...
    print('len(train_iterator)：',len(train_iterator))  # 1。只出现一次
...
def main():
    ...
    parser.add_argument(
        "--model_path",
        default='./models/bert-base-chinese',#chinese_electra_small_L-12_H-256_A-4',#electra_180g_large',#None,
        type=str,
        # required=True,
...
if __name__ == "__main__":
    torch.cuda.set_device(1)  # 0表示第一个GPU，1表示第二个GPU，以此类推
    main()
    print('Train Over !!!')

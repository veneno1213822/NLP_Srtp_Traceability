...
class OneKETrainer:
    ...
    def internal_data_to_train(self):
        ...
        results_train = get_train_data_nooption(datas_train, processer_train, converter_train, 4, False, "NER", "zh",
                                                source_train)  # 如果是训练数据转化任务，则将数据转化为训练数据格式
        ...
        results_test = get_test_data_nooption(datas_test, processer_test, 4, "NER", "zh",
                                              source_test)  # 如果是训练数据转化任务，则将数据转化为训练数据格式, split_num, task, language, source

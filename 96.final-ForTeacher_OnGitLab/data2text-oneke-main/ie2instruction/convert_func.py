...
def get_test_data_nooption(datas, processer, split_num, task, language, source):
    results = []
    for record in datas:
        iid = stable_hash(record['text'])
        task_record = processer.get_task_record(record)  
        ...
        if schemas is None:
            continue
        total_schemas = multischema_split_by_num_test(schemas, split_num)  
        for schema in total_schemas:
            sinstruct = multischema_construct_instruction(task, language, schema, record['text'])
            record2 = {
                'id': iid,
                'task': task,
                'source': source,
...
def get_train_data_nooption(datas, processer, converter, split_num, random_sort, task, language, source):
    results = []
    for record in datas:
        total_schemas = processer.negative_sample(record, split_num, random_sort)        
        task_record = processer.get_task_record(record)
        output_texts = convert_output(converter, record['text'], total_schemas, task_record)    # 按照split_num切分schema和output_text
        for schema, output_text in zip(total_schemas, output_texts):
            sinstruct = multischema_construct_instruction(task, language, schema, record['text'])
...
def process(options):
    converter = get_converter(options.task)(options.language, NAN='NAN') # 创建一个converter对象
    processer_class = get_processer(options.task)  # 创建processer_class对象
    ...
    if options.split == 'test':  
        results = get_test_data(datas, processer, options) # 如果是测试数据转化任务，则将数据转化为测试数据格式
    else:
        results = get_train_data(datas, processer, converter, options) # 如果是训练数据转化任务，则将数据转化为训练数据格式
...
if __name__ == "__main__":
    ...
    parse.add_argument("--cluster_mode", action='store_true', help="是否使用cluster模式, 负样本只包括难负样本+split_num个其他负样本") # 如果action='store_true'。则默认值为false　
